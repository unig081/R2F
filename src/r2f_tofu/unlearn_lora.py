...
from .models import add_lora, get_model_device, load_causal_lm, load_tokenizer, set_lora_trainable_filter
...
def train_lora_baseline(cfg: dict[str, Any], smoke: bool = False) -> dict[str, Any]:
    ...
    set_lora_trainable_filter(model, target_modules, train_layers=train_layers, train_modules=train_modules)

    trainable = [p for p in model.parameters() if p.requires_grad]
    if not trainable:
        raise RuntimeError("No LoRA parameters are trainable")
    optimizer = torch.optim.AdamW(trainable, lr=float(deep_get(cfg, "unlearning.lora_learning_rate", 1e-4)))
    ...
    for step, batch in enumerate(tqdm(loader, desc="LoRA-GA+GD-3B"), start=1):
        ...
        if not shape_report_written:
            ...
            save_shape_report(output_dir / "lora_shape_report.json", shape_report_from_records(lora_records))
